...
- **TaskManager.UI**  
  Contiene los formularios WinForms y se encarga únicamente de la interacción con el usuario.  
...
- **TaskManager.ViewModels**  
  ...
  La UI se comunica exclusivamente con esta capa.

- **TaskManager.Domain**  
  Contiene las entidades y enumeraciones del dominio, representando el modelo de datos del sistema.

- **TaskManager.Data**  
  Maneja el acceso a datos, repositorios y la inicialización de la base de datos.

Aunque WinForms no soporta MVVM de forma nativa, esta adaptación permite una separación clara de responsabilidades y facilita el mantenimiento y escalabilidad del proyecto.
...
Estos permisos no solo definirían las acciones que un usuario puede realizar (crear, editar, eliminar tareas o cambiar estados), sino que también permitirían **controlar el acceso a formularios o módulos del sistema**.

De esta forma, el sistema podría:

- Restringir el acceso a ciertos formularios según el perfil del usuario.
- Habilitar o deshabilitar funcionalidades futuras sin modificar la lógica principal.
- Facilitar la expansión del sistema a nuevos módulos manteniendo un control centralizado de permisos.
